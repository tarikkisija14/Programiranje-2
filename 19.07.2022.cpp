...
public:
	...
	Rjecnik operator()(int from, int to) {
...
public:
	...
	void DodajKomentarKarakteristike(Karakteristike k, const char* opis) {
		for (size_t i = 0; i < _komentariKarakteristika.getTrenutno(); i++)
		{
			if (_komentariKarakteristika.getElement1(i) == k)
				throw exception("vec dodano");
		}
		_komentariKarakteristika.AddElement(k, opis);
	}
...
public:
	...
	Rjecnik<Karakteristike, const char*> GetKupovineByKomentar(const char* komentar) {
		Rjecnik<Karakteristike, const char*> op;
