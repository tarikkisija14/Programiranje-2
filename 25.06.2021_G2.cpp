...
public:
    ...
    friend bool operator==(const Polaganje& p1, const Polaganje& p2) {
        if (p1._pojas != p2._pojas)
            return false;
        if (p1._polozeneTehnike.getTrenutno() != p2._polozeneTehnike.getTrenutno())
            return false;
        for (size_t i = 0; i < p1._polozeneTehnike.getTrenutno(); i++)
        {
            if (p1._polozeneTehnike.getElement1(i)!=p2._polozeneTehnike.getElement1(i)||
                p1._polozeneTehnike.getElement2(i)!=p2._polozeneTehnike.getElement2(i))
                return false;
        }
        return true;
    }
    ...
    float getProsjekPolaganje()const {
        ...
        {
            sum += _polozeneTehnike.getElement1(i)->getProsjekTehnika();
        }
        return sum / _polozeneTehnike.getTrenutno();
    }
...
bool ValidirajEmail(string email) {
    return regex_match(email, regex("\\d\\_\\w{3,}(\\.|\\_)?\\w{3,}@(karate.ba|edu.karate.ba|karate.com|karate.org|edu.karate.com|edu.karate.org)"));
...
public:
    KaratePolaznik(const char* imePrezime, string emailAdresa, string brojTelefona) {
        ...
        {
            _polozeniPojasevi.push_back(Pojas(i));
        }
    }
   ...
   - dodavanje tehnika za vise pojaseve ako ne postoji najmanje jedna tehnika za nizi pojas 
   (ne mozemo dodati tehniku za NARANDZASTI ako ne postoji niti jedna tehnika za ZUTI pojas)
   - dodavanje tehnika kod kojih je prosjecna ocjena manja od 3.5
   funkcija vraca true ili false u zavisnosti od (ne)uspjesnost izvrsenja
   */
    bool AddTehniku(Pojas pojas, Tehnika tehnika, const char* napomena) {
        if (pojas == BIJELI)
            return false;
        for (size_t i = 0; i < _polozeniPojasevi[pojas].GetTehnike().getTrenutno(); i++)
        {
            if (*_polozeniPojasevi[pojas].GetTehnike().getElement1(i) == tehnika)
                return false;
        }
        if (_polozeniPojasevi[pojas - 1].GetPojas() != BIJELI) {
            if (_polozeniPojasevi[pojas - 1].GetTehnike().getTrenutno() < 1)
                return false;
            if (tehnika.getProsjekTehnika() < 3.5f)
                return false;
        }
        _polozeniPojasevi[pojas].GetTehnike().AddElement(new Tehnika(tehnika), napomena);
    ...
    /*Kolekcija<Tehnika*, float>operator()( Datum d1, Datum d2) {
        Kolekcija<Tehnika*, float> op;
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
        {
            for (size_t j = 0; j < _polozeniPojasevi[i].GetTehnike().getElement1(i)->GetOcjene().getTrenutno(); j++)
            {
                for (size_t k = 0; k < _polozeniPojasevi[i].GetTehnike().getElement1(j)->GetOcjene().getTrenutno(); k++)
                {
                    Datum datum = _polozeniPojasevi[i].GetTehnike().getElement1(j)->GetOcjene().getElement1(k);
                    if (datum >= d1 && datum <= d2) {
                        op.AddElement(_polozeniPojasevi[i].GetTehnike().getElement1(j), 1.0f);
                        break;
                    }
                }
            }
    ...
    friend ostream& operator<< (ostream& COUT, KaratePolaznik& obj) {
        ...
        for (size_t i = 0; i < obj._polozeniPojasevi.size(); i++) {
            if (obj._polozeniPojasevi[i].GetTehnike().getTrenutno() > 0) {
