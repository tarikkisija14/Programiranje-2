        ...
        if (!_omoguciDupliranje) {
            for (size_t i = 0; i < *_trenutno; i++)
            {
                if (_elementi1[i] == el1 && _elementi2[i] == el2)
                    throw exception("nije dozvoljno dupliranje elemanata");
            }
        }
        T1* temp1 = _elementi1;
        T2* temp2 = _elementi2;
...
public:
    ...
    friend ostream& operator<<(ostream& COUT, const Komentar& obj) {
        ...
        COUT << "prosjek: " << obj.GetProsjekKomentara() << endl;
        return COUT;
    }
...
unique_ptr<char[]>GetNizKarakteraU(const char* tekst) {
    int vel = strlen(tekst) + 1;
...
public:
    Gost(const char* imePrezime, string emailAdresa, string brojPasosa) {
        ...
        _brojPasosa = ValidirajBrojPasosa(brojPasosa) ? brojPasosa : "NOTSTET";
    }
...
public:
    ...
    pair<int, int> GetBrojZnakova(string ime, const char* pretraga) {
        int ukupno = 0;
        int trazeno = 0;
        char znak;
        ifstream fajl(ime);
        if (fajl.is_open()) {
            while (fajl.get(znak)) {
                ukupno++;
                for (size_t i = 0; i < strlen(pretraga); i++)
                {
                    if(pretraga[i] == znak)
                        trazeno++;
                }
        ...
        fajl.close();
        return { ukupno,trazeno };
    }

    void testna(string ime) {
        ofstream fajl(ime);
        if (fajl.is_open()) {
            fajl << "denis+music*_";
        }
...
const char* GetOdgovorNaPrvoPitanje() {
    ...
    return "Odgovor -> Predusklovi da bi se ostvario polimorfizam su abstraktna kalsa s virutalnom metodom koju ce nasljediuit druga klasa koja ce imati svoiju implementaciju te metode nakon cega ako se naprave instance klasa poziv na istu metodu ce dati drugacije rezultate";
}
const char* GetOdgovorNaDrugoPitanje() {
    ...
    return "Odgovor -> Korsitenje kljucene rijeci abstract ili ciste vrituelne metode dovodi do toga da se ne moze napraviti instanca na klase najcesce ona sluzi kak baza u hijerahjiji nasljedjivanja.";
}
